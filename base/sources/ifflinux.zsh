__zplug::sources::ifflinux::check()
{
    local    repo="$1"
    local -A tags
...
__zplug::sources::ifflinux::install()
{
    local repo="$1"
...
__zplug::sources::ifflinux::update()
{
    local    repo="$1"
...
__zplug::sources::ifflinux::get_url()
{
    local repo="$1" url_format

    # git@ifflinux.iff.kfa-juelich.de:%s
    url_format="git@ifflinux.iff.kfa-juelich.de:i.heimbach@fz-juelich.de/${repo}"

    echo "$url_format"
}

__zplug::sources::ifflinux::load_plugin()
{
    local    repo="${1:?}"
...
__zplug::sources::ifflinux::load_command()
{
    local    repo="${1:?}"
    local -A tags default_tags
    local    src dst
...
__zplug::sources::ifflinux::load_theme()
{
    local    repo="${1:?}"
    local -A tags default_tags
    local -a themes_ext
